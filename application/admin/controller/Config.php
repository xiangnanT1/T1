...
class Config extends Controller
{
    ...
    public function clear()
    {
        ...
        Db::table('xy_reads')->where('uid', '>', 1)->delete();
        Db::table('xy_reward_log')->where('uid', '>', 1)->delete();
        Db::table('xy_verify_msg')->where('type', '>', 0)->delete();
        if (in_array(1,$ids)) {
            Db::table('xy_users')->where('uid', '>', 1)->delete();
        }
        if (in_array(2,$ids)) {
        ...
        if (in_array(9,$ids)) {
            ...
            foreach ($data as $key => $val) {
                Db::table('xy_convey')->where('id',$val['id'])->delete();
            }
    ...
    public function storage_sms(){
       
        $codes = db('system_sms')->where('id','<',4)->select();
        $this->is_manual = db('system_sms')->where('id',4)->find();
      
        $this->applyCsrfToken();
        $post = Request::post();
        if($post){
            foreach ($post as $key => $value) {
                Db::name('system_sms')->where('position',$key)->setField('status',$value);
            }
